 ...
 * 품목별 처리 단계를 만든다.
 *
 * 단계 개수는 구조이므로 지금 정하고, 실제 행동 설명은 박재웅의 사실 검수 뒤에 채운다.
 * 검수 결과 단계 수가 달라지면 이 숫자를 조정한다. 3개 또는 4개만 가능하다.
 */
function steps(itemId: ItemId, count: 3 | 4): CatalogStep[] {
  return Array.from({ length: count }, (_, index) => {
    const id = String(index + 1).padStart(2, '0');
    return { id, text: localized(itemId, `step${id}`, {}) };
  });
}
 ...
 */
function image(itemId: ItemId) {
  return {
    image: `/images/items/${itemId}.webp`,
    imageAlt: localized(itemId, 'imageAlt', {}),
  };
}
