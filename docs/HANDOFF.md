...
- 기본 브랜치: `main`
- 현재 단계: 제품 설계 승인 및 구현 계획 완료, 앱 구현 시작 전
- 제품명: `K-SORT`
...
현재 저장소에는 승인된 설계, 구현 계획, AI 프롬프트, 보고서/PPT 작성 자료가 있다. 실제 React 앱, Cloudflare Worker, 영상·이미지 에셋은 김현민 개발 단계에서 추가한다.
...
- [ ] Cloudflare 계정 로그인 가능 여부 확인
- [ ] Google AI Studio에서 Gemini API 키 발급 여부 확인
- [ ] 저장소 clone 후 `main`이 최신인지 확인
- [ ] 기능 브랜치 생성
- [ ] 구현 계획의 Task 1부터 체크박스 순서대로 진행
- [ ] 각 Task의 테스트 실패와 성공을 확인한 후 작은 커밋 생성
- [ ] API 키는 `.dev.vars` 또는 Cloudflare secret에만 저장
...
의존성 설치 후에는 구현 계획 Task 1의 파일과 테스트를 작성한다. 한 번에 전체 앱을 생성하지 말고 Task별 테스트와 커밋을 유지한다.
...
- 프런트: React + Vite + TypeScript, HashRouter, GitHub Pages
- API: Cloudflare Worker, Gemini `gemini-3.6-flash`
- 전역 언어: `ko | en | zh | vi`
- 채팅: `POST /api/chat`, 500자, 최근 6개, 분당 10회
- 스캔: `POST /api/scan`, 1.5MB, 최대 5개 물체, 분당 5회
- 콘텐츠: 16종 도감, 품목별 3~4개 과정 이미지, FAQ 20개
...
1. docs/superpowers/specs/2026-08-06-k-sort-design.md
2. docs/superpowers/plans/2026-08-06-k-sort-implementation.md
3. docs/HANDOFF.md
4. docs/AI_PROCESS_AND_PROMPTS.md

구현 계획의 아직 완료되지 않은 첫 Task부터 테스트 주도로 진행해. 범위를 추가하거나 설계 결정을 임의로 바꾸지 말고, Task마다 실패 테스트 → 최소 구현 → 전체 관련 테스트 → 커밋 순서를 지켜. 실제 API 키를 코드, 로그, 문서에 쓰지 마. 이미 존재하는 팀원 변경은 보존하고, 시작 전에 git status와 최근 커밋을 확인해.
...
- 배포 URL, Worker URL, 최종 커밋과 태그가 README에 기록된다.
