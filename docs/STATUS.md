...
**마지막 갱신**: 2026-08-07 · 현민

## 현재 단계

- **큰 마일스톤**: **콘텐츠를 전부 채웠다.** 자리 표시 0개, 릴리스 게이트 3개 통과.
  다섯 화면이 4개 언어로 동작하고 챗봇이 실제로 답한다.
- **웹앱**: <https://kodol05.github.io/make-upload/>
- **로컬 시연**: <http://localhost:5173/make-upload/> — 발표는 노트북에서 하므로 이쪽이 기준이다
- **API**: <https://make-upload.vercel.app> — `POST /api/chat`, `POST /api/scan`
- **진행 중 Task**: Task 12 (최종 검증·증거 수집·발표 동결)
- **Blocker**: 없음.

### ⚠️ 번역은 원어민 검수를 받지 못했다

영어·중국어·베트남어 문안은 개발자가 작성했고 원어민이 확인하지 않았다. 뜻이 틀린
곳은 없다고 보지만 어색한 표현이 남아 있을 수 있다. 유학생 검토를 받을 기회가 생기면
`shared/catalog.ts`, `shared/faqs.ts`, `src/i18n/strings.ts` 세 파일만 보면 된다.

### 로컬에서 AI 기능을 쓰려면

`VITE_API_BASE_URL`은 빌드 시점에 박히므로 로컬에는 따로 넣어야 한다. 저장소 루트에
`.env.local`을 만들고 아래 한 줄을 적은 뒤 개발 서버를 다시 띄운다. `.env*`는
`.gitignore`에 있어 커밋되지 않는다.

```
VITE_API_BASE_URL=https://make-upload.vercel.app
```

포트는 **5173이어야 한다.** API의 CORS 허용 목록에 `http://localhost:5173`만 들어
있어서, 포트가 밀리면 AI 기능이 막힌다.

## 지금 · 다음

| 지금 | 다음 |
|------|------|
| Task 12: 스크린샷·리허설·secret 검색·태그 | 발표 |
...
```
자리 표시 0개 남음 · 출처 URL 5/5개 확인됨
```

**한국어·영어·중국어·베트남어를 모두 채웠다.** 도감 16종, FAQ 20개, 출처 5개,
화면 문자열 98개, 검색용 별칭까지 포함한다. 릴리스 게이트 세 개도 켜 두었으므로
문안이나 출처가 비면 `npm run check`가 실패해 배포가 막힌다.
...
문안 근거는 기후에너지환경부 「재활용품 분리배출 가이드라인」과 생활폐기물 분리배출
누리집(분리배출.kr) 품목사전이다. 출처 5개는 직접 열어 200을 확인했다.

### 확인하면서 바로잡은 것

일반적으로 알려진 것과 다른 부분이라 발표에서 쓸 만하다.

| 품목 | 흔한 통념 | 실제 |
|------|-----------|------|
| 투명 페트병 뚜껑 | 떼서 버린다 | **닫아서 버린다.** 뚜껑(PE·PP)은 뜨고 몸통(PET)은 가라앉아 세척 공정에서 갈라진다 |
| 빨대 | 종량제 봉투 | **재활용폐기물** (누리집 품목사전) |
| 종이 컵라면 용기 | 일반쓰레기 | **재활용폐기물**. 블로그 다수가 틀리게 적어 놓았다 |

환경부가 **기후에너지환경부**로 바뀌어 `me.go.kr`이 `mcee.go.kr`로 넘어간다는 것도
이때 확인했다.

### 번역 방침

유학생이 현장에서 마주치는 한국어 단어는 괄호로 병기한다.

```
standard garbage bag (종량제 봉투)
community service centre (주민센터)
```

편의점에서 봉투를 사고 수거함을 찾을 때 결국 그 글자를 읽어야 하기 때문이다.
문안을 더할 때도 이 방식을 유지한다.
...
**테스트 246개 전부 통과, skip 0개.** `npm run check` (lint + test + build) 통과 상태.
...
1. 이미지 16장과 영상·자막을 받아 `public/`에 넣는다. 없어도 화면은 동작한다
2. 360px·데스크톱 스크린샷, 리허설 3회, secret 검색, 릴리스 태그
3. 여유가 되면 번역 원어민 검수

## 최근 결정 · 변경

- 2026-08-07 · **콘텐츠 4개 언어 완성.** 자리 표시 0개, 릴리스 게이트 3개 활성화 (PR #28)
- 2026-08-07 · 검색이 고른 언어와 한국어를 항상 함께 찾도록 수정. 수거함에 한국어가 적혀 있어서다
- 2026-08-06 · Pages 배포 실패의 원인은 deploy-pages의 10분 상한. 큐 대기가 그보다 길었다 (PR #27)
