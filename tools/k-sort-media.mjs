...
export async function probeMedia(file, { dryRun = false } = {}) {
  ...
  return {
    video: video && { width: Number(video.width), height: Number(video.height), duration: Number(video.duration ?? parsed.format.duration) },
    audio: audio && { sampleRate: Number(audio.sample_rate), channels: Number(audio.channels), duration: Number(audio.duration ?? parsed.format.duration) },
  };
}
...
async function writeRecord(name, records) {
  ...
  await writeFile(join(RECORD_DIR, `${name}.json`), `${JSON.stringify({ generatedAt: new Date().toISOString(), records }, null, 2)}\n`);
}
...
export async function generateTts({ clipIds = [], force = false, dryRun = false, key = process.env.FAL_KEY } = {}) {
  ...
  try {
    for (const request of requests) {
      ...
      const valid = await exists(output) && (await probeMedia(output)).audio?.sampleRate === 48000;
      if (valid && !force) { records.push({ requestId: request.id, outputPath: request.outputFile, status: 'skipped' }); continue; }
      ...
      records.push({ requestId: request.id, outputPath: request.outputFile, sourceUrl });
    }
...
export async function generateVideo({ clipIds = [], force = false, dryRun = false, key = process.env.FAL_KEY } = {}) {
  ...
  for (const clip of clips) {
    ...
    if (valid && !force) { records.push({ requestId: clip.id, outputPath: clip.outputFile, status: 'skipped' }); continue; }
    ...
    records.push({ requestId: clip.id, outputPath: clip.outputFile, sourceUrl });
  }
...
export function buildAssemblyCommands({ concatFile, narrationFilter: filter, bgm, subtitleFile, fontFile }) {
  const audioInputs = []; // supplied by assembleMaster; this builder is deterministic and path-independent.
  const base = ['-y', '-f', 'concat', '-safe', '0', '-i', concatFile, ...audioInputs];
  const clean = { args: [...base, '-filter_complex', filter, '-map', '0:v:0', '-map', '[aout]', '-shortest', '-r', '24', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-ar', '48000', '-movflags', '+faststart'], output: 'public/media/k-sort-guide-clean.mp4' };
...
export async function assembleMaster({ bgm, dryRun = false } = {}) {
  ...
  const commands = buildAssemblyCommands({ concatFile, narrationFilter: filter, bgm, subtitleFile: 'public/subtitles/ko.vtt', fontFile });
  const narrationInputs = tts.requests.flatMap((request) => ['-i', resolve(ROOT, request.outputFile)]);
  const bgmInputs = bgm ? ['-stream_loop', '-1', '-i', resolve(ROOT, bgm)] : [];
  try {
    await run('ffmpeg', [...commands.clean.args.slice(0, 6), ...narrationInputs, ...bgmInputs, ...commands.clean.args.slice(6), CLEAN_MASTER], { dryRun });
...
export async function validateMedia({ dryRun = false } = {}) {
  ...
  for (const request of tts.requests) {
    ...
    audio.push({ id: request.id, file: request.outputFile, valid: Boolean(probe?.audio?.sampleRate === 48000 && probe.audio.channels === 1) });
  }
  return { videos, audio, valid: videos.every((item) => item.valid) && audio.every((item) => item.valid) };
}
